package viper.server.frontends.lsp.file.branchTree

import viper.silver.utility.Common.PrintWriter
...
class BranchTree {
  ...
  protected[branchTree] def extend(branchConditions : Vector[(String, Boolean)], isResultFatal: Boolean) : Unit  = {
    (this, branchConditions) match {
      case (b:Branch, bcs) if bcs.nonEmpty =>
        var currBranch = b
        var negated = branchConditions.head._2
        var tail = branchConditions.tail
        var next = true
        while (tail.nonEmpty && next) {
          next = false
          val headExp = tail.head._1
          (currBranch.left, currBranch.right) match {
            case (lb@Branch(exp,_,_,_,_),_) if headExp == exp && negated =>
              currBranch.leftResFatalCount = incrementIfFatal(currBranch.leftResFatalCount,isResultFatal)
              currBranch = lb
              next = true
            case (_,rb@Branch(exp,_,_,_,_)) if headExp == exp && !negated =>
  ...
  private def buildTreeStr() : (Vector[String], Int, Int) = {
    this match {
      case Branch(exp, left, right, leftErrCount, rightErrCount) =>
        val expStrLen = exp.length

        var boxMiddle = "│ " + exp + (if (even(exp.length)) " " else "") + " │"
...
object BranchTree {
  val DotFilePath = s"${java.nio.file.Paths.get(System.getProperty("java.io.tmpdir"), ".vscode")}/BranchTree.dot"

  @tailrec
  private def generatePathRec(expressions: Vector[(String, Boolean)],
                              errorCount: Int,
                              result: BranchTree): BranchTree = {
    expressions.length match {
      case 0 => result
      case _ =>
        val (lastExp, negated) = expressions.last
        negated match {
          case true =>
            generatePathRec(expressions.init, errorCount, Branch(lastExp, result, Leaf, errorCount, 0))
          case _ =>
  ...
  @tailrec
  private def generateRec(exploredPaths: Vector[(Vector[(String, Boolean)],Boolean)], result: BranchTree): BranchTree = { // result.instanceOf[Branch] must hold
  ...
  def generate(exploredPaths: Vector[(Vector[(String, Boolean)],Boolean)]): Option[BranchTree] = {
    exploredPaths.length match {
      case 0 => None
      case _ =>
        val (expressions, isResultFatal) = exploredPaths.head
...
case class Branch(var exp : String,
                  var left: BranchTree,
